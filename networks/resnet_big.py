...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, is_last=False, norm=nn.BatchNorm2d):
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, in_channel=3, zero_init_residual=False, norm=nn.BatchNorm2d):
        ...
        for m in self.modules():
            ...
            elif isinstance(m, (nn.BatchNorm2d, nn.GroupNorm, norm)):
                nn.init.constant_(m.weight, 1)
                try:
                    nn.init.constant_(m.bias, 0)
                except AttributeError: # RMSNorm has no bias
                    pass
    ...
    def _make_layer(self, block, planes, num_blocks, stride, norm=nn.BatchNorm2d):
...
class SupConResNet(nn.Module):
    ...
    def __init__(self, name='resnet50', head='mlp', feat_dim=128, norm=nn.BatchNorm2d):
        ...
        self.encoder = model_fun(norm=norm)
        if head == 'linear':
